...
142l. Greek word sections show a collapsed "What does this mean?" accordion beneath the raw Morphology field, decoding the code into a plain-English grammatical breakdown — expand-in-place, native `<details>`/`<summary>` (no custom JS toggle, same pattern as reference.js's Related-tab "show all" groups), collapsed by default, nothing overlaid or hidden elsewhere on the page — language.js:renderMorphDecodeAccordion, css:`.language-morph-decode*`
